...
const createSendToken = (user, statusCode, res) => {
    ...
    const cookieOptions =  {
        expires: new Date(Date.now() + process.env.JWT_COOKIE_EXPIRES_IN * 24 * 6 * 6 * 1000),
        httpOnly: true
    }
...
//   });
  ...
  // Forgot password for HNG COVER LETTER TASK
  exports.forgotPassword = catchAsync(async (req, res, next) => {
        ...
        if (!user) {
         ...
         // 2) Generate the random reset token and Save to DB
         const otp = user.createPasswordResetToken(); 5564
        await user.save({ validateBeforeSave: false });

        // 3) Send OTP to user's email
        const message = `OTP FOR VERIFICATION : ${otp}`
  ...
  exports.validateOtp = catchAsync(async (req, res, next) => {
    ...
    // 2) Check If Otp in email == otp in DB
        const user = await User.findOne({ passwordResetToken: hashedOtp, passwordResetExpires: { $gt:Date.now() } });
        console.log(user);
        if (!user) {
    ...
    // 3) redirect to Reset password page
        const resetURL = `${req.protocol}://${req.get(
           'host'
        )}/api/v1/users/resetPassword/${otp}`; 
